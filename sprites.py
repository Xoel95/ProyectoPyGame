...
class Spritesheet:
	...
	def get_sprite(self, x, y, width, height):
		# función que coje el sprite especificado dentro de la hoja de sprites y lo prepara para el juego
...
class Attack(pygame.sprite.Sprite):
	...
	def animate(self):
		...
		if self.facing == 'down':
			if self.y_change == 0:
				self.image = self.game.attack_spritesheet.get_sprite(3, 2, self.width, self.height)
			else:
				self.image = down_animations[math.floor(self.animation_loop)]
				self.animation_loop += 0.1
				if self.animation_loop >= 3:
		...
		if self.facing == 'up':
			if self.y_change == 0:
				self.image = self.game.attack_spritesheet.get_sprite(3, 34, self.width, self.height)
			else:
				self.image = up_animations[math.floor(self.animation_loop)]
				self.animation_loop += 0.1
				if self.animation_loop >= 3:
		...
		if self.facing == 'left':
			if self.x_change == 0:
				self.image = self.game.attack_spritesheet.get_sprite(3, 98, self.width, self.height)
			else:
				self.image = left_animations[math.floor(self.animation_loop)]
				self.animation_loop += 0.1
				if self.animation_loop >= 3:
		...
		if self.facing == 'right':
			if self.x_change == 0:
				self.image = self.game.attack_spritesheet.get_sprite(3, 66, self.width, self.height)
			else:
				self.image = right_animations[math.floor(self.animation_loop)]
				self.animation_loop += 0.1
				if self.animation_loop >= 3:
